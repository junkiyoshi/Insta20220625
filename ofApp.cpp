	...
	*/
	...
	if (ofGetFrameNum() % 4 == 0) {
		...
		this->rotation_list.push_back(glm::vec3(
			...
			ofMap(ofNoise(noise_param.x, ofGetFrameNum() * 0.008), 0, 1, -360, 360),
			ofMap(ofNoise(noise_param.y, ofGetFrameNum() * 0.008), 0, 1, -360, 360),
			ofMap(ofNoise(noise_param.z, ofGetFrameNum() * 0.008), 0, 1, -360, 360)));
		*/
	}

	for (int i = this->radius_list.size() - 1; i > -1; i--) {

		this->radius_list[i] += 2.5;
	...
	for (int i = 0; i < this->radius_list.size(); i++) {
		...
		auto alpha = this->radius_list[i] < 50 ? 255 : ofMap(this->radius_list[i], 50, 250, 255, 0);
		this->setRingToMesh(this->face, this->frame, glm::vec3(), this->rotation_list[i], this->radius_list[i], this->radius_list[i] * 0.1, ofColor(0, alpha), ofColor(255, alpha));
	}
}
